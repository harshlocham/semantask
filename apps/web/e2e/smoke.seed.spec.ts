...
import { readE2eSeed } from "./seed-io";
...
test("alice can sign in and the coordination board is reachable", async ({ page }) => {
    ...
    // Do not use page.request here. CI runs next start with NODE_ENV=production,
    // so auth cookies are Secure and Playwright's APIRequestContext will not
    // send them to http://127.0.0.1 — GET /api/conversations returns 401.
    const { conversationId, taskTitle } = readE2eSeed();
    expect(conversationId).toBeTruthy();

    await page.goto(`/inbox/board?conversationId=${conversationId}`);
    await expect(page.getByTestId("inbox-subnav")).toBeVisible();
    await expect(page.getByTestId("work-board-card-title")).toHaveText(taskTitle, {
        timeout: 20_000,
    });
});
